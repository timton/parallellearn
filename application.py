...
def index():
    ...
    for version in new_versions:
        rows = Project.query.filter(Project.id == version.project_id).all()
    ...
    popular_versions = Version.query.order_by(Version.rating.desc()).limit(5).all()
    ...
    if "user_id" in session:
        rows = User.query.filter(User.id == session.user_id).all()
    ...
    for comment in comments:
        if comment.user_id == 0:
            ...
            rows = User.query.filter(User.id == comment.user_id).all()
...
def quick_practice():
    ...
    if request.method == "POST":
        ...
        from_projects_ids = []
        rows = Version.query.filter(Version.language == from_language).all()
        for row in rows:
        ...
        for id in from_projects_ids:
            rows = Version.query.filter(and_(Version.project_id == id,
        ...
        for id in possible_projects_ids:
            rows = Versions.query.filter(and_(Version.project_id == id, Version.language == from_language)).all()
        ...
        for id in from_versions_ids:
            rows = Line.query.filter(Line.version_id == id).all()
        ...
        for line in from_lines:
            ...
            rows = Version.query.filter(and_(Version.project_id == line.project_id, Version.language=to_language)).all()

            # pick the to-version at random
            to_version = choice(rows)

            # get the corresponding to-line
            rows = Line.query.filter(and_(Line.version_id == to_version.id, Line.line_index=line.line_index)).all()
        ...
        for line in from_lines:
            ...
            if from_lines.index(line) == 0:
                rows = Project.query.filter(Project.id == line.project_id).all()
...
def register():
    ...
    if request.method == "POST":
        ...
        rows = User.query.filter(User.email == request.form.get("email")).all()

        if row:
            return apology("try another email")

        # ensure username uniqueness
        rows = User.query.filter(User.username == request.form.get("username")).all()
...
def log_in():
    ...
    if request.method == "POST":
        ...
        rows = User.query.filter(or_(User.email == eoru, User.username == eoru)).all()
...
def view_details():
    ...
    # fetch the user
    user = User.query.filter(User.id == session.user_id).all()
...
def change_password():
    ...
    if request.method == "POST":
        ...
        User.query.filter(User.id == session.user_id).update({"hash": pwd_context.hash(request.form.get("new_password"))})
...
def change_email():
    ...
    if request.method == "POST":
        ...
        rows = User.query.filter(User.email == request.form.get("new_email")).all()

        if len(rows) > 0:
            return apology("try another email")

        # update new email
        User.query.filter(User.id == session.user_id).update({"email": request.form.get("new_email")})
...
def change_username():
    ...
    if request.method == "POST":
        ...
        row = User.query.filter(User.username == request.form.get("new_username")).all()
...
def new_project_metadata():
    ...
    if request.method == "POST":
        ...
        if session.new_project.type.lower() == "series":
            rows = Project.query.filter(and_(Project.type == session.new_project.type.lower(),
...
def new_project_formatting():
    ...
    if request.method == "POST":
        ...
        version_ids = []
        rows = Version.query.filter(Version.project_id == session.new_project.project_id).order_by(Version.id.desc()).all()
...
def upload_existing():
    ...
    if request.method == "POST":
        ...
        rows = Project.query.filter(Project.id == project_id).all()
...
def existing_project_versions():
    ...
    if request.method == "POST":
        ...
        for i in range(session.existing_project.number_of_versions):
            ...
            try:
                version = Version(project_id=session.existing_project.project_id,
        ...
        # https://stackoverflow.com/questions/3940128/how-can-i-reverse-a-list-in-python
        version_ids = []
        rows = Version.query.filter(Version.project_id == session.existing_project.id).order_by(Version.id.desc()).all()
...
def view_project(id):
    ...
    # get project metadata
    rows = Project.query.filter(Project.id == id).all()
    project = rows[0]
    ...
    rows = Version.query.filter(Version.project_id == id).all()
    language_list = []
    sources = []
    for row in rows:
        ...
        credit = {}
        if row.source != None:
            users = User.query.filter(User.id == row.user_id).all()
...
def view_history():
    ...
    started_projects = Resumable.query.filter(Resumable.user_id == session.user_id).all()
    ...
    if len(started_projects) > 0:
        for project in started_projects:
            ...
            rows = Version.query.filter(Version.id == project.from_version_id).all()
            project.from_language = rows[0].language

            # add "to_version" metadata
            rows = Version.query.filter(Version.id == project.to_version_id).all()
            project.to_language = rows[0].language

            # add project metadata
            rows = Project.query.filter(Project.id == project.project_id).all()
    ...
    uploaded_projects = Project.query.filter(Project.user_id == session.user_id).all()
    ...
    if len(uploaded_projects) > 0:
        for project in uploaded_projects:
            ...
            rows = Version.query.filter(and_(Version.project_id == project.id, Version.user_id == session.user_id)).all()
    ...
    uploaded_versions = Version.query.filter(Version.user_id == session.user_id).all()
    ...
    if len(not_user_project_ids) > 0:
        ...
        for id in not_user_project_ids:
            ...
            rows = Version.query.filter(and_(Version.project_id == id, Version.user_id == session.user_id)).all()
...
def prepare_deletion():
    ...
    if request.method == "POST":
        ...
        project.versions_to_delete = Version.query.filter(and_(Version.project_id == project_id,
...
def delete():
    ...
    if request.method == "POST":
        ...
        if request.form.get('delete_project'):
            ...
            rows = Version.query.filter(Version.project_id == project_id).all()
            for row in rows:
            ...
            rows = Resumable.query.filter(Resumable.project_id == project_id).all()
            for row in rows:
                if row.user_id != session.user_id:
        ...
        # if deleting versions one by one
            ...
            rows = Version.query.filter(Version.id == version_id).all()
            if rows[0].user_id != session.user_id:
            ...
            other_versions = Version.query.filter(and_(Version.project_id == project_id,
    ...
    if request.method == "POST":
        ...
        # else if user chooses what he wants to change specifically
            ...
            if request.form.get('change_project_type'):
                ...
                rows = Project.query.filter(and_(Project.type == new_type,
            ...
            if request.form.get('change_to_series'):
                ...
                rows = Project.query.filter(and_(Project.type == "series",
                                                 Project.title == project.title,
                                                 Project.author == project.author,
            ...
            if request.form.get('change_project_name'):
                ...
                if project.type == "series":
                    rows = Project.query.filter(and_(Project.type == project.type,
                                                     Project.title == title,
...
def prepare_practice():
    ...
    if request.method == "POST":
        ...
        rows = Project.query.filter(Project.id == project_id).all()
        project = rows[0]
        project.versions = Version.query.filter(Version.project_id == project_id).all()

        # fetch the user for each version
        for version in project.versions:
            rows = User.query.filter(User.id == version.user_id).all()
...
def project_practice():
    ...
    if request.method == "POST":
        ...
        if request.form.get('start_practice'):
            ...
            rows = Version.query.filter(Version.id == to_version_id).all()
            ...
            project.starting_line = 0
            if "user_id" in session:
                rows = Resumable.query.filter(and_(Resumable.user_id == session.user_id,
            ...
            project.from_lines = Line.query.filter(Line.version_id == from_version_id)
                                           .order_by(Line.id.asc()).all()
            project.to_lines = Line.query.filter(Line.version_id == to_version_id)
                                         .order_by(Line.id.asc()).all()
        ...
        # else if project practice resumed from the account history page
            ...
            rows = Resumable.query.filter(Resumable.id == resume_id).all()
            starting_line = rows[0].progress
            from_lines = Line.query.filter(Line.version_id == rows[0].from_version_id)
                                   .order_by(Line.id.asc()).all()
            to_lines = Line.query.filter(Line.version_id == rows[0].to_version_id)
                                 .order_by(Line.id.asc()).all()
            project_id = rows[0].project_id
...
def browse():
    ...
    books = Project.query.filter(Project.type == "book").all()
    movies = Project.query.filter(Project.type == "movie").all()
    songs = Project.query.filter(Project.type == "song").all()
...
def edit_line():
    ...
    if request.method == "POST":
        ...
        if request.form.get('line_to_edit'):
            ...
            rows = Line.query.filter(Line.id == bad_line_id).all()
            bad_version_id = rows[0].version_id
            rows = Version.query.filter(Version.id == bad_version_id).all()
            ...
            rows = Line.query.filter(Line.id == good_line_id).all()
...
def view_notifications():
    ...
    versions = Version.query.filter(Version.user_id == session.user_id).all()

    # fetch the proposed line corrections for these versions
    corrections = []
    for version in versions:
        rows = Correction.query.filter(Correction.version_id == version.id).all()
        for row in rows:
    ...
    for correction in corrections:
        notification = {}
        rows = Project.query.filter(Project.id == correction.project_id).all()
        notification.title = rows[0].title
        rows = Line.query.filter(Line.id == correction.context_line_id).all()
        notification.context_line = rows[0].line
        from_version_id = rows[0].version_id
        rows = Version.query.filter(Version.id == from_version_id).all()
        notification.from_language = rows[0].language
        rows = User.query.filter(User.id == correction.user_id).all()
        notification.user = rows[0].username
        notification.suggested_line = correction.correction
        rows = Line.query.filter(Line.id == correction.line_id).all()
...
def action_correction():
    ...
    if request.method == "POST":
        ...
        if request.form.get('implement_correction'):
            ...
            rows = Correction.query.filter(Correction.id == correction_id).all()
...
def project_download():
    ...
    if request.method == "POST":
        ...
        lines = Line.query.filter(Line.version_id == selected_versions[0]).all()
        ...
        if len(selected_versions) > 1:
            for version in selected_versions:
                ...
                lines = Line.query.filter(Line.version_id == version).all()
...
def has_notifications():
    ...
    if session:
        if "user_id" in session:
            ...
            user_versions = []
            rows = Version.query.filter(Version.user_id == session.user_id).all()

            for row in rows:
                user_versions.append(row.id)

            notifications = []
            rows = Correction.query.all()
            for row in rows:
...
def prepare_to_rate():
    ...
    if request.method == "POST":
        ...
        rows = Rating.query.filter(and_(Rating.user_id == session.user_id,
        ...
        project_id = rows[0].project_id
        rows = User.query.filter(User.id == user_id).all()
        username = rows[0].username
        rows = Project.query.filter(Project.id == project_id).all()
        version = rows[0]
...
def rate():
    ...
    if request.method == "POST":
        ...
        rows = Rating.query.filter(Rating.version_id == version_id).all()
        new_rating = 0.0
        for row in rows:
